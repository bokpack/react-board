...
const NavBar = ({  setIsAuthenticated, onSearch, searchQuery, setSearchQuery, selectedField, setSelectedField, onHomeClick}) => {
    ...
    return (
        <nav className="relative flex flex-wrap justify-between items-center p-4 bg-indigo-400 text-white">
            ...
            <button onClick={onHomeClick} className="text-lg font-bold hover:text-gray-300">
                홈
            </button>
                {/* 검색바 & 드롭다운 */}
                <div className="flex items-center gap-4 md:gap-6 order-3 md:order-2 w-full md:w-auto justify-center md:justify-center mt-4 md:mt-0">
                    <DropDown toggleMenu="선택" options={dropDownOptions} selectedField={selectedField} setSelectedField={setSelectedField} />
                    <SearchBar searchQuery={searchQuery} setSearchQuery={setSearchQuery}  onSearch={(query) => onSearch(query, selectedField)} />
                </div>
